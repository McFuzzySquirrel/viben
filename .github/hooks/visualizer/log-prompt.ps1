...
if (Test-Path $_vizEmitScript) {
  try {
    $_vizPayload = (ConvertTo-Json @{ prompt = $(if ($env:PROMPT) { $env:PROMPT } else { '' }) } -Compress)
    & $_vizEmitScript -EventType "userPromptSubmitted" -Payload $_vizPayload -SessionId $(if ($env:SESSION_ID) { $env:SESSION_ID } else { "run-$PID" }) 2>&1 | Out-Null
