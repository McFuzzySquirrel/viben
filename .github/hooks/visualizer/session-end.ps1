...
if (Test-Path $_vizEmitScript) {
  try {
    $_vizPayload = (ConvertTo-Json @{ reason = $(if ($env:REASON) { $env:REASON } else { 'unknown' }) } -Compress)
    & $_vizEmitScript -EventType "sessionEnd" -Payload $_vizPayload -SessionId $(if ($env:SESSION_ID) { $env:SESSION_ID } else { "run-$PID" }) 2>&1 | Out-Null
