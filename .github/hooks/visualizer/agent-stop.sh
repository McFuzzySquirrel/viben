...
if [ -x "${REPO_ROOT}/.visualizer/emit-event.sh" ]; then
  _VIZ_PAYLOAD=$(jq -nc --arg agent "${AGENT_NAME:-${SUBAGENT_NAME:-${AGENT_TYPE:-${AGENT_TASK_NAME:-${AGENT_DISPLAY_NAME:-${SUBAGENT_DISPLAY_NAME:-${TASK_DESC:-unknown}}}}}}}" --arg task "${TASK_DESC:-}" --arg detail "${TASK_DESC:-${AGENT_DESCRIPTION:-${SUBAGENT_DESCRIPTION:-${AGENT_MESSAGE:-${MESSAGE:-${SUMMARY:-${RESULT:-${REASON:-}}}}}}}}" --arg agentType "${AGENT_TYPE:-}" --arg agentId "${AGENT_ID:-${SUBAGENT_ID:-}}" --arg reason "${REASON:-}" --arg message "${AGENT_MESSAGE:-${MESSAGE:-${SUMMARY:-${RESULT:-${TASK_DESC:-${REASON:-}}}}}}" '{"agentName":$agent,"taskDescription":$task,"description":$detail,"reason":$reason,"message":$message,"summary":$message} + (if ($agentType|length)>0 then {"agentType":$agentType} else {} end) + (if ($agentId|length)>0 then {"agentId":$agentId} else {} end)' 2>/dev/null || echo '{}')
  "${REPO_ROOT}/.visualizer/emit-event.sh" agentStop "${_VIZ_PAYLOAD}" "${SESSION_ID:-run-$$}" >&2 || true
fi
