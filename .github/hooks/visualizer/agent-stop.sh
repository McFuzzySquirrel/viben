...
if [ -x "${REPO_ROOT}/.visualizer/emit-event.sh" ]; then
  _VIZ_PAYLOAD=$(jq -nc --arg agent "${AGENT_NAME:-${SUBAGENT_NAME:-${AGENT_DISPLAY_NAME:-${SUBAGENT_DISPLAY_NAME:-${TASK_DESC:-unknown}}}}}" --arg reason "${REASON:-}" --arg message "${AGENT_MESSAGE:-${MESSAGE:-${SUMMARY:-${RESULT:-}}}}" '{"agentName":$agent,"reason":$reason,"message":$message,"summary":$message}' 2>/dev/null || echo '{}')
