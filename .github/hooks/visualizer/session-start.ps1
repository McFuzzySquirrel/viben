...
if (Test-Path $_vizEmitScript) {
  try {
    $_vizPayload = (ConvertTo-Json @{ source = $(if ($env:SOURCE) { $env:SOURCE } else { 'unknown' }) } -Compress)
    & $_vizEmitScript -EventType "sessionStart" -Payload $_vizPayload -SessionId $(if ($env:SESSION_ID) { $env:SESSION_ID } else { "run-$PID" }) 2>&1 | Out-Null
