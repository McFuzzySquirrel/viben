...
if [ -x "${REPO_ROOT}/.visualizer/emit-event.sh" ]; then
  _VIZ_PAYLOAD=$(jq -nc --arg message "${MESSAGE:-unknown error}" --arg code "${CODE:-}" '{"message":$message,"code":$code}' 2>/dev/null || echo '{}')
  "${REPO_ROOT}/.visualizer/emit-event.sh" errorOccurred "${_VIZ_PAYLOAD}" "${SESSION_ID:-run-$$}" >&2 || true
fi
