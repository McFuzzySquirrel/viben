...
if (Test-Path $_vizEmitScript) {
  try {
    $_vizPayload = (ConvertTo-Json @{ agentName = $(if ($env:AGENT_NAME) { $env:AGENT_NAME } elseif ($env:SUBAGENT_NAME) { $env:SUBAGENT_NAME } elseif ($env:AGENT_DISPLAY_NAME) { $env:AGENT_DISPLAY_NAME } else { 'unknown' }); reason = $(if ($env:REASON) { $env:REASON } else { '' }); message = $(if ($env:AGENT_MESSAGE) { $env:AGENT_MESSAGE } elseif ($env:MESSAGE) { $env:MESSAGE } else { '' }); summary = $(if ($env:SUMMARY) { $env:SUMMARY } else { '' }) } -Compress)
