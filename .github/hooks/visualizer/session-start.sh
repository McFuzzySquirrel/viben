...
if [ -x "${REPO_ROOT}/.visualizer/emit-event.sh" ]; then
  _VIZ_PAYLOAD=$(jq -nc --arg source "${SOURCE:-unknown}" '{"source":$source}' 2>/dev/null || echo '{}')
  "${REPO_ROOT}/.visualizer/emit-event.sh" sessionStart "${_VIZ_PAYLOAD}" "${SESSION_ID:-run-$$}" >&2 || true
fi
