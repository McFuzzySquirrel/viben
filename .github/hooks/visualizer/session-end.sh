...
if [ -x "${REPO_ROOT}/.visualizer/emit-event.sh" ]; then
  _VIZ_PAYLOAD=$(jq -nc --arg reason "${REASON:-unknown}" '{"reason":$reason}' 2>/dev/null || echo '{}')
  "${REPO_ROOT}/.visualizer/emit-event.sh" sessionEnd "${_VIZ_PAYLOAD}" "${SESSION_ID:-run-$$}" >&2 || true
fi
